...
watch(() => authStore.user, (user) => {
  ...
  else if (!user && !authStore.isLoading) {
    showPublic.value = true;
    loadPublicPlaylists()
  }
})
...
watch(() => authStore.isLoading, (loading) => {
  if (!loading && !authStore.user && !hasFetched) {
    showPublic.value = true;
    loadPublicPlaylists()
  }
})
...
async function loadSmartPlaylists() {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : 'Failed to load smart playlists',
      color: 'error',
      icon: 'i-lucide-alert-circle'
    })
  } finally {
...
const spDeleteOpen = computed({
  ...
  set: (v: boolean) => {
    if (!v) spDeleteTarget.value = null
  },
})
...
const spPreviewOpen = computed({
  ...
  set: (v: boolean) => {
    if (!v) spPreviewTarget.value = null
  },
})
...
async function loadSmartPreview(sp: SmartPlaylist) {
  ...
  try {
    ...
    if (mounted && thisGen === spPreviewGen) spPreviewLoading.value = false
  }
}
  ...
  <UContainer class="py-6 max-w-4xl space-y-6">
    ...
    <template v-else-if="authStore.user">
      ...
      <template v-if="activePlaylist">
        ...
        <UCard>
          <template #header>
            <div class="flex items-center justify-between gap-3 flex-wrap">
              <div>
                ...
                <p v-if="activePlaylist.description" class="text-sm text-muted mt-0.5">{{
                    activePlaylist.description
                  }}</p>
              </div>
              <div class="flex items-center gap-2 flex-wrap">
                <UBadge :label="activePlaylist.is_public ? 'Public' : 'Private'"
                        :color="activePlaylist.is_public ? 'success' : 'neutral'" variant="subtle" size="xs"/>
                <UButton icon="i-lucide-pencil" label="Edit" size="xs" variant="outline" color="neutral"
                         @click="openEdit(activePlaylist)"/>
                <UButton icon="i-lucide-copy" label="Duplicate" size="xs" variant="outline" color="neutral"
                         :loading="copyingPersonalId === activePlaylist.id" @click="copyPlaylist(activePlaylist)"/>
                <UButton icon="i-lucide-trash-2" label="Clear Items" size="xs" variant="outline" color="warning"
                         :loading="clearingId === activePlaylist.id" @click="clearPlaylist(activePlaylist)"/>
                <UDropdownMenu :items="[[
                  ...
                  <UButton icon="i-lucide-download" label="Export" size="xs" variant="outline" color="neutral"
          ...
          <div v-else-if="!activePlaylist.items || activePlaylist.items.length === 0"
              ...
              <div class="flex items-start gap-2 min-w-0">
                ...
                <div class="min-w-0">
                  ...
                  <div class="flex items-center gap-2 mt-2">
                    <UBadge :label="pl.is_public ? 'Public' : 'Private'" :color="pl.is_public ? 'success' : 'neutral'"
                            variant="subtle" size="xs"/>
                    <span class="text-xs text-muted">{{ (pl.items?.length ?? 0) }} items</span>
                    <span v-if="pl.modified_at"
                          class="text-xs text-muted">· {{ new Date(pl.modified_at).toLocaleDateString() }}</span>
                  </div>
                </div>
              </div>
              <div v-if="!selectMode" class="flex items-center gap-1">
                <UButton icon="i-lucide-pencil" aria-label="Edit playlist" size="xs" variant="ghost" color="neutral"
    ...
    <template v-else>
      <div class="flex items-center justify-between flex-wrap gap-3">
        ...
        <UButton icon="i-lucide-log-in" label="Log in to create playlists" variant="outline" color="neutral"
                 to="/login"/>
      </div>
