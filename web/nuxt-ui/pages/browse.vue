 ...
 */
import {useMediaApi, useTagsApi} from '~/composables/useApiEndpoints'
...
<template>
  <UContainer class="py-6 max-w-5xl">
    <header class="space-y-2 mb-5">
      ...
      <p class="text-sm text-muted">Pick a few tags to narrow the library — pair the AND mode to find items that match
        every tag, or OR mode for anything matching one.</p>
    </header>
    ...
    >
      <div class="flex-1 min-w-[200px]">
        <p class="text-sm font-semibold">{{ selected.size }} tag<span v-if="selected.size !== 1">s</span> selected ·
          {{ mode === 'and' ? 'AND' : 'OR' }} mode</p>
        <p class="text-xs text-muted">
          <UIcon v-if="previewLoading" name="i-lucide-loader-2" class="size-3 animate-spin inline"/>
          <span v-else-if="previewCount !== null">{{ previewCount.toLocaleString() }} item<span
              v-if="previewCount !== 1">s</span> match</span>
