...
async function handleAction(fn: () => Promise<unknown>, successMsg: string, actionKey?: string) {
  ...
  try {
    ...
    const cleared = new Set(actionBusy.value);
    cleared.delete(key);
    actionBusy.value = cleared
  }
}
  ...
  <div class="space-y-6">
    ...
    <UCard v-if="system">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm mb-4">
        ...
        <div v-if="system.build_date"><span class="text-muted">Built:</span>
          {{ new Date(system.build_date).toLocaleDateString() }}
        </div>
