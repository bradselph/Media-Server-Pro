...
import type {AdminPlaylistStats, Playlist} from '~/types/api'
  ...
  result = [...result].sort((a, b) => {
    ...
    switch (sortKey.value) {
      ...
      case 'items':
        ...
        break
      case 'is_public':
        cmp = Number(a.is_public) - Number(b.is_public);
        break
      case 'created_at':
...
watch([search, visibilityFilter, sortKey, sortDir, perPage], () => {
  page.value = 1
})
...
async function executeBulkDelete() {
  ...
  try {
    ...
    toast.add({
      title: `Deleted ${res.success} playlist${plural}${failedMsg}`,
  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-3 gap-3">
      ...
      <UCard :ui="{ body: 'p-3' }">
        <p class="text-xl font-bold text-highlighted">
          {{ stats?.public_playlists ?? playlists.filter(p => p.is_public).length }}</p>
        <p class="text-xs text-muted mt-1">Public</p>
      </UCard>
      <UCard :ui="{ body: 'p-3' }">
        <p class="text-xl font-bold text-highlighted">
          {{ stats?.total_items ?? playlists.reduce((s, p) => s + (p.items?.length ?? 0), 0) }}</p>
    ...
    <div class="flex flex-wrap gap-2 items-center justify-between">
      ...
      <div class="flex gap-2 items-center">
        ...
        <UButton icon="i-lucide-refresh-cw" aria-label="Refresh playlists" variant="ghost" color="neutral"
                 @click="load"/>
      </div>
    </div>
    ...
    >
      <template #body>
        <p>Are you sure you want to delete <strong>{{ selected.size }}</strong> selected
