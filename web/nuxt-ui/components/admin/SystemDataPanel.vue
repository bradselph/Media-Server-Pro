...
async function loadDbStatus() {
  ...
  try {
    dbStatus.value = await adminApi.getDatabaseStatus()
  } catch {
    dbStatusError.value = true
  }
}
...
async function restoreBackup(id: string) {
  ...
  const next = new Set(backupBusy.value);
  next.add(`restore-${id}`);
  backupBusy.value = next
  try {
    ...
    const cleared = new Set(backupBusy.value);
    cleared.delete(`restore-${id}`);
    backupBusy.value = cleared
  }
}
  ...
  <div class="space-y-6">
    ...
    <div class="space-y-3">
      ...
      <UCard :ui="{ body: 'p-4' }">
        <div class="flex flex-wrap items-end gap-4">
          <UFormField label="Retention count"
                      description="Number of backup files to keep (older ones are deleted automatically)">
            <UInput
        ...
        >
          <template #file_count-cell="{ row }">
            <span class="text-sm tabular-nums">
              ...
              <span v-if="row.original.errors?.length" class="text-error ml-1">({{
    ...
    >
      ...
      <template #footer>
        ...
        <UButton color="warning" label="Restore"
                 :loading="!!confirmRestoreId && backupBusy.has(`restore-${confirmRestoreId}`)"
    ...
    >
      ...
      <template #footer>
        ...
        <UButton color="error" label="Delete"
                 :loading="!!confirmDeleteId && backupBusy.has(`delete-${confirmDeleteId}`)"
                 @click="() => { const id = confirmDeleteId; confirmDeleteId = null; if (id) deleteBackup(id) }"/>
      </template>
    </UModal>
          ...
          <div v-if="dbQueryResult" class="space-y-1 text-sm">
            ...
            <div class="overflow-x-auto rounded border border-default">
              <table v-if="dbQueryResult.columns?.length" class="min-w-full text-xs font-mono">
                ...
                <tr>
                  <th v-for="col in dbQueryResult.columns" :key="col"
                      class="px-3 py-1.5 text-left font-medium text-muted whitespace-nowrap">{{ col }}
                  </th>
                </tr>
                </thead>
                <tbody class="divide-y divide-default">
                <tr v-for="(row, ri) in dbQueryResult.rows" :key="ri" class="hover:bg-muted/30">
                  <td v-for="(col, colIdx) in dbQueryResult.columns" :key="col"
                      class="px-3 py-1 whitespace-nowrap max-w-xs truncate"
                      :title="String((row as unknown[])[colIdx] ?? '')">
