...
import type {ModuleHealth, ServerStatus} from '~/types/api'
...
async function loadStatus() {
  ...
  try {
    ...
    if (mounted) statusLoading.value = false
  }
}
...
async function showModuleDetail(name: string) {
  ...
  try {
    const detail = await adminApi.getModuleHealth(name)
    if (!mounted) return
    moduleDetail.value = detail
  } catch (e: unknown) {
    notifyError(e, 'Failed to load module health')
  } finally {
    if (mounted) moduleDetailLoading.value = false
  }
}
...
<template>
  <div class="space-y-4">
    <div class="flex justify-end">
      <UButton icon="i-lucide-refresh-cw" aria-label="Refresh status" variant="ghost" color="neutral"
      ...
      <UCard v-if="serverStatus">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
          ...
          <div><span class="text-muted">Started:</span>
            {{ serverStatus.start_time ? new Date(serverStatus.start_time).toLocaleString() : '—' }}
          </div>
