...
export function useHLS(
  ...
  async function attachHLS(url: string) {
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, (_event: unknown, data: { levels: Array<{ height: number; width: number; bitrate: number; videoCodec?: string }> }) => {
    ...
    hls.on(Hls.Events.FRAG_LOADED, (_event: unknown, data: { frag: { stats: { loaded: number; loading: { start: number; end: number } } } }) => {
